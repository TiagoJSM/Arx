		...
		public static void FetchNodes() 
		{
			...
			foreach (Assembly assembly in scriptAssemblies) 
			{
				foreach (Type type in assembly.GetTypes ().Where (T => T.IsClass && !T.IsAbstract && T.IsSubclassOf (typeof (Node)))) 
				{
					...
					{
                        var scriptable = ScriptableObject.CreateInstance(type.Name);
                        Node node = scriptable as Node; // Create a 'raw' instance (not setup using the appropriate Create function)
